...
[![Accuracy](https://img.shields.io/badge/Tumor%20Classifier-78%25-00F2FF?style=for-the-badge)](/models/model.h5)
...
2. **Stage 2 — Tumor Classifier**: A fine-tuned **MobileNetV2** model retrained on the Kaggle Brain Tumor dataset achieving **78% test accuracy** across 4 classes — Glioma, Meningioma, Pituitary tumor, and No Tumor.
...
│
├── 📄 train.py                        # Tumor classifier training script
│   ├── Loads from Training/ and Testing/
...
│
├── 📁 models/
│   ├── model.h5                       # Retrained tumor classifier (78% acc)
...
Input (128×128×3)
       ...
       ▼
┌─────────────────────────────────┐
│   Data Augmentation (training)  │  RandomFlip + RandomRotation
│   RandomZoom + RandomTranslation│  (identity pass at inference)
└─────────────┬───────────────────┘
              │
              ▼
┌─────────────────────────────────┐
│   MobileNetV2 (Frozen Base)     │  ← Pre-trained on ImageNet
│   1,280 feature maps @ 4×4     │    Transfer Learning
└─────────────┬───────────────────┘
              │
              ▼
...
|--------|-------|
| **Test Accuracy** | 78.00% |
| **Precision (Macro)** | 78.57% |
| **Recall (Macro)** | 78.00% |
| **F1 Score (Macro)** | 77.56% |
...
This will save the retrained model to `models/model.h5` and print:
